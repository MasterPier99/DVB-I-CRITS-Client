...
function parseContentGuideSource(src) {
  ...
  if (src) {
    ...
    var SIEURI = src.getElementsByTagName("ScheduleInfoEndpoint")[0].getElementsByTagName("URI")[0];
    if ( !SIEURI) SIEURI = src.getElementsByTagName("ScheduleInfoEndpoint")[0].getElementsByTagName("dvbi-types:URI")[0];
	  ...
	  if (idx != -1) {
		...
		var app3, mediaUri3;
                if(howRelated3 == "urn:dvb:metadata:cs:LinkedApplicationCS:2019:1.1") {
                    app3 = {};
                    mediaUri3 =  relatedMaterial3[k].getElementsByTagNameNS(howRelatedNamespace,"MediaLocator")[0].getElementsByTagNameNS(howRelatedNamespace,"MediaUri")[0];
            ...
            if(serviceInstances[j].getElementsByTagName("DASHDeliveryParameters").length > 0 ) {
                   try {
                    var tryURI = serviceInstances[j].getElementsByTagName("URI")[0];
                    if (!tryURI) tryURI = serviceInstances[j].getElementsByTagName("dvbi-types:URI")[0];
